...
int blob_decode_t::output(int &n,char ** &s_arr,int *&len_arr)
{
	...
	if(parser_pos+(int)sizeof(u32_t)>current_len) {TSK_DEBUG_INFO("failed 0\n");return -1;}

	n=(int)read_u32(input_buf+parser_pos);
	if(n>max_blob_packet_num) {TSK_DEBUG_INFO("failed 1\n");return -1;}
	s_arr=output_buf;
	len_arr=output_len;
	...
	for(int i=0;i<n;i++)
	{
		if(parser_pos+(int)sizeof(u16_t)>current_len) {TSK_DEBUG_INFO("failed2 \n");return -1;}
		len_arr[i]=(int)read_u16(input_buf+parser_pos);
		parser_pos+=(int)sizeof(u16_t);
		if(parser_pos+len_arr[i]>current_len) {TSK_DEBUG_INFO("failed 3 %d  %d %d\n",parser_pos,len_arr[i],current_len);return -1;}
...
int fec_encode_manager_t::input(char *s,int len/*,int &is_first_packet*/)
{
	...
	if(fec_par.mode==0&& s!=0 &&counter==0)
	{
		...
		{
			TSK_DEBUG_ERROR("message too long ori_len=%d out_len=%d fec_mtu=%d,ignored\n",len,out_len,fec_par.mtu);
			return -1;
		}
	}
	if(fec_par.mode==1&&s!=0&&len>fec_par.mtu)
	{
		TSK_DEBUG_ERROR("mode==1,message len=%d,len>fec_mtu,fec_mtu=%d,packet may not be delivered\n",len,fec_par.mtu);
		//return -1;
	}
	if(s==0&&counter==0)
	{
		TSK_DEBUG_ERROR("unexpected s==0&&counter==0\n");
		return -1;
	}
    ...
    if(about_to_fec)
	{
    	char ** blob_output=0;
    	int fec_len=-1;
    	int actual_data_num = 0;
    	int actual_redundant_num =0;
    	...
    	if(counter==0)
    	{
    		TSK_DEBUG_ERROR("unexpected counter==0 here\n");
    		return -1;
    	}
    	...
    	if(0)
    	{
			...
			for(int i=0;i<actual_data_num;i++)
			{
				...
				{
					TSK_DEBUG_INFO("0x%02x,",(u32_t)(unsigned char)input_buf[i][j]);
				}
				printf("},\n");
				//TSK_DEBUG_INFO("")
			}
		...
		if(0)
		{
			...
			for(int i=0;i<actual_data_num+actual_redundant_num;i++)
			{
				...
				for(int j=0;j<8+fec_len;j++)
				{
					TSK_DEBUG_INFO("0x%02x,",(u32_t)(unsigned char)output_buf[i][j]);
				}
				printf("},\n");
				//TSK_DEBUG_INFO("")
			}
    ...
    else
    {
    	if(encode_fast_send&&s!=0&&fec_par.mode==1)
    	{
    		...
    		if(0)
    		{
				...
				{
					TSK_DEBUG_INFO("0x%02x,",(u32_t)(unsigned char)output_buf[0][j]);
				}
				printf("\n");
    		}
    	}
...
int fec_decode_manager_t::input(char *s,int len)
{
	...
	//TSK_DEBUG_INFO("input\n");

	if(len<0)
	{
		TSK_DEBUG_ERROR("len<0\n");
		return -1;
	}
	...
	if(type==1)
	{
		...
		{
			TSK_DEBUG_ERROR("type==1&&len<2\n");
			return -1;
		}
		if(data_num==0&&(int)( read_u16(s+tmp_idx)+sizeof(u16_t))!=len)
		{
			TSK_DEBUG_ERROR("inner_index<data_num&&read_u16(s+tmp_idx)+sizeof(u16_t)!=len    %d %d\n",(int)( read_u16(s+tmp_idx)+sizeof(u16_t)),len);
			return -1;
		}
	}
	...
	{
		TSK_DEBUG_ERROR("unexpected type==0&&data_num==0\n");
		return -1;
	}
	if(data_num+redundant_num>=max_fec_packet_num)
	{
		TSK_DEBUG_ERROR("data_num+redundant_num>=max_fec_packet_num\n");
		return -1;
	}
	if(!anti_replay.is_vaild(seq))
	{
		TSK_DEBUG_INFO("!anti_replay.is_vaild(seq) ,seq =%u\n",seq);
		return 0;
	}

	if(mp[seq].fec_done!=0)
	{
		TSK_DEBUG_INFO("fec already done, ignore, seq=%u\n",seq);
		return -1;
	}

	if(mp[seq].group_mp.find(inner_index)!=mp[seq].group_mp.end() )
	{
		TSK_DEBUG_INFO("dup fec index\n");//duplicate can happen on  a normal network, so its just log_debug
		return -1;
	}
	...
	if(data_num!=0)
	{
		...
		else
		{
			...
			{
				TSK_DEBUG_ERROR("unexpected mp[seq].data_num!=data_num||mp[seq].redundant_num!=redundant_num||mp[seq].len!=len\n");
				return -1;
			}
		}
	}

	//TSK_DEBUG_INFO("mp.size()=%d index=%d\n",mp.size(),index);

	if(fec_data[index].used!=0)
	{
		u32_t tmp_seq=fec_data[index].seq;
		map<u32_t, fec_group_t>::iterator tmp_it=mp.find(tmp_seq);
		...
		if(tmp_it!=mp.end())
		{
			...
			{
				if(debug_fec_dec){
					TSK_DEBUG_INFO("[dec][failed]seq=%08x x=%d y=%d cnt=%d\n",tmp_seq,x,y,cnt);
				}
				else{
					TSK_DEBUG_INFO("[dec][failed]seq=%08x x=%d y=%d cnt=%d\n",tmp_seq,x,y,cnt);
				}
		...
		if(tmp_seq==seq)
		{
			TSK_DEBUG_ERROR("unexpected tmp_seq==seq ,seq=%d\n",seq);
			return -1;
		}
	}
	...
	{//lyz add.
		...
		else
		{
			if(mp[seq].data_num!=-1)
			{
				...
				{
					TSK_DEBUG_ERROR("inner_mp.size()>data_num+1\n");
					anti_replay.set_invaild(seq);
					goto end;
				}
		...
		if(about_to_fec)
		{
			...
			if(type==0)
			{
				...
				{
					TSK_DEBUG_ERROR("blob_decode failed\n");
					//ready_for_output=0;
					anti_replay.set_invaild(seq);
					goto end;
				}
			...
			else//type==1
			{
				...
				for(map<int,int>::iterator it=inner_mp.begin();it!=inner_mp.end();it++)
				{
					...
					{
						TSK_DEBUG_ERROR("fec_data[it->second].len<(int)sizeof(u16_t)");
						data_check_ok=0;
					}
				...
				{
					data_check_ok=0;
					TSK_DEBUG_ERROR("max_len!=mp[seq].len");
				}
				if(data_check_ok==0)
				{
					//ready_for_output=0;
					TSK_DEBUG_ERROR("data_check_ok==0\n");
					anti_replay.set_invaild(seq);
					goto end;
				}
				for(map<int,int>::iterator it=inner_mp.begin();it!=inner_mp.end();it++)
		...
		else //not about_to_fec
		{
			...
			if(decode_fast_send)
			{
				if(type==1&&data_num==0)
				{
					...
					{
						TSK_DEBUG_ERROR("len mismatch %d %d\n",output_len_arr_buf[0],check_len);
					}
					output_s_arr=output_s_arr_buf;
					output_len_arr=output_len_arr_buf;
